...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        //line chart
        $linecharts = Notification::select(DB::raw("sum(quantity) as count"), DB::raw("MONTHNAME(created_at) as month_name"))
                    ->whereYear('created_at', date('Y'))
                    ->groupBy(DB::raw("month_name"))
                    ->orderBy('created_at','ASC')
                    ->pluck('count', 'month_name');
        ...
        return view('dashboard',compact('products','reports','labels', 'data','linelabels','line','avaibility','performance','quality','oee'))->with('i',(request()->input('page',1)-1)*5);
    }
}
