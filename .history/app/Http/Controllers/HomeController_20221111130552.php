...
class HomeController extends Controller
{
    ...
    {
        $reports = Report::latest()->simplePaginate(5);
        ...
        //performance
        $performancethismonth = Product::where('due_date', '>=', Carbon::now())->count();
        ...
        //bar chart
        $charts = Notification::select(DB::raw("sum(quantity) as count"), 
        ...
        //line chart
        $linecharts = Product::select(DB::raw("count(*) as count"), 
        DB::raw("MONTHNAME(created_at) as month_name"))
                    ->whereYear('created_at', date('Y'))
                    ->groupBy(DB::raw("month_name"))
                    ->orderBy('created_at','ASC')
