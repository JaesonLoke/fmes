...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $avaibilitylastmonth = Product::where('status','Running')->whereMonth('created_at', '=', Carbon::now())->count();
        $avaibilitysum = Product::where('status','Running')->whereMonth('created_at', '=', Carbon::now())->count();

        $avaibility = $lastmonth == 0 ? 0 : $avaibilitythismonth / $avaibilitysum;

        $avaibility = round($percentage*100,2);
