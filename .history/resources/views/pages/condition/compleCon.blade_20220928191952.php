...
@if ($row['status'] == 'delayed' || $row['status'] == 'pending' || $row['status'] == 'emergency' )  
      ...
      <div class="progress">
        <div class="progress-bar bg-warning" role="progressbar" aria-valuenow="{{if($row['quantity']==0){ $value == 0$value =  $row['completion'] / $row['quantity'] * 100}else {} }}" aria-valuemin="0" aria-valuemax="100" style="width: {{$value}}%;"></div>
      </div>
