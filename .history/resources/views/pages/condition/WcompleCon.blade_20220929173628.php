...
@if ($row['status'] == 'delayed' || $row['status'] == 'pending' || $row['status'] == 'emergency' )  
      ...
      <div class="progress">
        ...
        {{-- aria-valuenow="{{ $value = DB::table('products')->where('id', $_GET['id'])->AVG('completion' / 'quantity') ; }}"  --}}
...
@elseif ($row['status'] == 'completed' || $row['status'] == 'new' || $row['status'] == 'Running')  
      ...
      <div class="progress">
        ...
        aria-valuenow="{{$value = (DB::table('products')->where('id', $_GET['id'])->AVG('completion') / (DB::table('products')->where('id', $_GET['id'])->AVG('completion') ; }}" 
        aria-valuemin="0" aria-valuemax="100" style="width: {{$value}}%;"></div>
      </div>
