...
@elseif ($row['status'] == 'completed' || $row['status'] == 'new' || $row['status'] == 'Running')  
      ...
      <span class="completion mr-2"> {{$value = DB::table('products')->select(DB::raw('round(AVG(quantity/quantity),0) as quantity'))->where('workorder_id', $_GET['id'])->value("quantity")  }}%</span>
