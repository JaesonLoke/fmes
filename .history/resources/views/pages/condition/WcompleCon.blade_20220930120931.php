...
@elseif ($row['status'] == 'completed' || $row['status'] == 'new' || $row['status'] == 'Running')  
      ...
      <span class="completion mr-2"> {{$value = DB::table('products')->select('completion')->where('workorder_id', $_GET['id']) / DB::table('products')->where('workorder_id', $_GET['id'])->select('quantity') }}%</span>
