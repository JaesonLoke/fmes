...
@if ($row['status'] == 'delayed' || $row['status'] == 'pending' || $row['status'] == 'emergency' )  
      ...
      <div class="progress">
        ...
        aria-valuenow="{{ $value == 0 ? 0 : DB::table('products')->where('id', $_GET['id'])->AVG('completion' / 'quantity') ; }}" 
...
@elseif ($row['status'] == 'completed' || $row['status'] == 'new' || $row['status'] == 'Running')  
      ...
      <div class="progress">
        ...
        aria-valuenow="{{$value == 0 ? 0 : DB::table('products')->where('id', $_GET['id'])->AVG((int)'completion' / (int)'quantity') ; }}" 
        aria-valuemin="0" aria-valuemax="100" style="width: {{$value}}%;"></div>
      </div>
