...
@elseif ($row['status'] == 'completed' || $row['status'] == 'new' || $row['status'] == 'Running')  
      ...
      <div class="progress">
        ...
        {{-- aria-valuenow="{{$value = DB::table('products')->select('completion')->where('workorder_id', $_GET['id']) / DB::table('products')->where('id', $_GET['id'])->select('quantity') }}"  --}}
        aria-valuemin="0" aria-valuemax="100" {{--style="width: {{}}%;"--}}></div>
      </div>

      <span class="completion mr-2"> {{$value = DB::table('products')->where('workorder_id', $_GET['id'])->select('AVG(completion)')->get()}}%</span>
